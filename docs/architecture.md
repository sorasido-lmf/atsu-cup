...
- `settings.html`の`?reauth=1`(バナーの「再ログイン」からの遷移)は、**`requestAnimationFrame`をやめて`renderButton()`の解決後に実行する**。理由は2つ: (1) rAFのコールバックは**タブが非表示だと発火しない**ので、スマホで「再ログイン」をタップした直後に別アプリへ移ると、戻ってきてもスクロールも強調もされなかった。今は`visibilitychange`で表示されるまで待つ (2) rAFの時点ではGISのボタンがまだ非同期描画されておらず`signinArea`が空なので、高さ0の要素を`block:'center'`で中央に寄せてしまい、その後ボタンが現れた分だけ位置がずれていた
