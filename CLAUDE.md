...
- 合わせて`pruneTournamentsGoneFromServer()`を追加し、**以前に一度でもリモートから取り込んだことがある(`remoteSnapshots`に記録がある)大会が、サーバー側でid自体無くなっていれば、この端末のキャッシュからも取り除く**(スプレッドシートの作り直しでidが変わった場合の回復)。未同期の新規作成大会は対象外なので安全(＝オフライン等でサーバーへまだ一度も反映できていない大会が、通常のページ読み込みで誤って消えることはない)。これは`loadFromData()`(毎回のページ読み込み)から呼ぶため、ログインを待たずに効く。
...
- **巻き戻し**: `resetMatchResult`と、通常の`pickWinner`で選び直した場合の両方で、`match.bye && match.a && match.b`(=本物のシードではなくこの機能で立てられたbyeフラグ)なら`bye`を`false`に戻す。本物の1回戦シードは`b===null`なので誤って巻き込まれない。
- **D&D除外**: `slotDndEligible`/`cardMoveEligible`は、この状態(`bye:true`かつ`b!==null`)を通常の決着済み対戦と同じ「ロック」対象として扱う(本物の空きシード`b===null`のみドラッグ対象を許可する既存条件に、`b===null`チェックを追加して区別)。
