...
**日付入力欄**: `.date-field-row`で**縦積み**にし、枠内右端に📅を重ねた(`.date-input`)。横並びだと375pxで潰れて読めなかった。⚠️ **`appearance:none`は外さないこと**(iOS Safariのネイティブ日付UIが`width:100%`を無視してはみ出す実機バグの対策)。そのためネイティブのカレンダーアイコンが出ず、**自前の要素を重ねている**。ネイティブの`::-webkit-calendar-picker-indicator`は非表示にしているので、**デスクトップで開く導線が消える。ラッパのclickで`showPicker()`を呼んで補っている**(未対応ブラウザでは例外を握りつぶす)。
